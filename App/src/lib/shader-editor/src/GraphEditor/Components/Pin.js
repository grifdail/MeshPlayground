...
function OutputPin({children, type,index, colorByType = {}, ...props}) {
  return <Group y={index*PIN_HEIGHT} {...props}>
    <circle fill="white"  stroke={colorByType[type]} strokeWidth="3" cx={NODE_WIDTH} cy={PIN_HEIGHT * 0.5} r={PIN_HEIGHT*0.3}/>
    <text textAnchor="end" x={NODE_WIDTH - PIN_HEIGHT*0.5} y={PIN_HEIGHT*0.5} alignmentBaseline="middle">{children}</text>
  </Group>
}

function InputPin({children, type, index, colorByType = {}, ...props}) {
  return <Group y={index*PIN_HEIGHT} {...props}>
    <circle fill="white" stroke={colorByType[type]} strokeWidth="3" cx={0} cy={PIN_HEIGHT * 0.5} r={PIN_HEIGHT*0.3}/>
